...
```bash
sgeventhub$ sam build --use-container
```
...
```bash
sgeventhub$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
sgeventhub$ sam local start-api
sgeventhub$ curl http://localhost:3000/
```
...
```bash
sgeventhub$ sam logs -n HelloWorldFunction --stack-name sgeventhub --tail
```
...
```bash
sgeventhub$ pip install -r tests/requirements.txt --user
# unit test
sgeventhub$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
sgeventhub$ AWS_SAM_STACK_NAME="sgeventhub" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "sgeventhub"
```
